...
# Lab specific
LAB_FILES_ARRAY=("arm/cpuid/main.c" "arm/sliding_puzzle/sliding_puzzle_asm.S" "x86/cpuid/main.c" "x86/popcnt/main.c")
LAB_DIRS_ARRAY=()
...
tar --exclude='*.o' --exclude='core' --exclude='*.su' --exclude='*.bin' --exclude='.cproject' --exclude='.project' --exclude='.settings' -czf "${archive_name}" ${LAB_FILES_ARRAY[*]} ${LAB_DIRS_ARRAY[*]} &> /dev/null
