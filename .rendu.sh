...
# Lab specific
LAB_FILES_ARRAY=("x86/directives/string_asm.S" "x86/frame/frame.S" "x86/gpio_fields/gpio_asm.S")
LAB_DIRS_ARRAY=("x86")
...
tar --exclude="main" --exclude='*.o' --exclude='core' --exclude='*.su' --exclude='*.bin' --exclude='.cproject' --exclude='.project' --exclude='.settings' -czf "${archive_name}" ${LAB_FILES_ARRAY[*]} ${LAB_DIRS_ARRAY[*]} &> /dev/null
